/*
Write a function that takes in a string of one or more words,
and returns the same string, but with all five or more 
letter words reversed (Just like the name of this Kata). 
Strings passed in will consist of only letters and spaces. 
Spaces will be included only when more than one word is present.

Examples: spinWords( "Hey fellow warriors" ) => returns 
"Hey wollef sroirraw" spinWords( "This is a test") => returns 
"This is a test" spinWords( "This is another test" )=> returns 
"This is rehtona test"
*/

// MY SOLUTION

function spinWords(string){
  // grab array reference to each word
  const str_array = string.split(' ')
  
  // check if each word length => 5 and reverse
  str_array.forEach((word, index)=>{
    if (word.length >= 5){
      str_array[index] = word.split('').reverse().join('')
    }
  })
  
  return str_array.join(' ')
}

// OTHER SOLUTIONS

function spinWords(words){  // concatenated version of my answer
  return words.split(' ').map(function (word) {
    return (word.length > 4) ? word.split('').reverse().join('') : word;
  }).join(' ');
}
