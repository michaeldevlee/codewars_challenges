...
function narcissistic(value) {  
  // get each digit of the value
  const string_array = value.toString().split('')
  const int_array = string_array.map((el)=>Number(el))
  const num_of_digits = string_array.length
  
  const value_reduce = int_array.reduce((prev,el)=>{
    return (prev + el**num_of_digits)},0)

  return value_reduce === value
}
