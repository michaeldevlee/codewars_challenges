...
function likes(names) {
    // check if array has any names in it
  if (names.length > 0){
    // if there are names, check how many there are and structure the return string
    const name_count = names.length
    if (name_count == 1){
      ...
      return names[0] + ', ' + names[1] + ' and ' + `${name_count-2}` + ' others like this'
    }
...
function likes(names) { // uses switch statements
  names = names || [];
  switch(names.length){
